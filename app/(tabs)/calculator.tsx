...
export default function Calculator() {
    ...
    const aliquotaInss =
        salario > 0 ? (inss / salarioCopy) * 100 : 0;

    const aliquotaIr =
        salarioCopy > 0
            ? (imposto / salarioCopy) * 100
            : 0;
    ...
    const percentual =
        salarioCopy > 0
            ? (totalDescontos / salarioCopy) * 100
            : 0;

    const salarioLiquido =
        salarioCopy - inss - imposto;
    ...
    function calculateImposto(
        ...
        if (baseOriginal <= 7350) {
            ...
            imposto = Math.max(
                0,
                imposto - descontoCalculado
            );

            setDesconto(
                Number(
                    descontoCalculado.toFixed(2)
                )
            );
        }
